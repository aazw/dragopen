...
document.addEventListener("mousedown", (e) => {
	if (isDebug) {
		console.log(
			`mousedown: pageX: ${e.pageX}, pageY: ${e.pageY}, isDragging: ${isDragging}`,
		);
	}
...
document.addEventListener("mousemove", (e) => {
	...
	if (isDebug) {
		console.log(
			`mousemove: pageX: ${e.pageX}, pageY: ${e.pageY}, isDragging: ${isDragging}`,
		);
	}
...
document.addEventListener("mouseup", (e) => {
	if (isDebug) {
		console.log(
			`mouseup: pageX: ${e.pageX}, pageY: ${e.pageY}, isDragging: ${isDragging}`,
		);
	}
	...
	if (selectionBox) {
		...
		if (selectedLinks.length > 0) {
			...
			try {
				...
				console.error(
					"the links can be not opened because background process is invalidated.",
				);
				console.error(e);
			}
		}
